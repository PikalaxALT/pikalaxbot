...
class PokeApi(aiosqlite.Connection):
    ...
    async def get_species_sprite_url(self, mon: 'PokeapiModel.classes.PokeomnSpecies'):
        sprites = await self._execute(lambda: json.loads(mon.pokemon_sprites[0].sprites))
        options = (
            ('front_default',),
    ...
    @staticmethod
    def get_default_pokemon_sync(mon: 'PokeapiModel.classes.PokemonSpecies'):
        return mon.pokemons.get(is_default=True)

    async def get_default_pokemon(self, mon: 'PokeapiModel.classes.PokemonSpecies'):
        return await self._execute(PokeApi.get_default_pokemon_sync, mon)

    async def get_mon_types(self, mon: 'PokeapiModel.classes.PokemonSpecies') -> list['PokeapiModel.classes.Type']:
        return await self._execute(
            lambda: [ptype.type for ptype in PokeApi.get_default_pokemon_sync(mon).pokemon_types]
        )
    ...
    async def get_mon_matchup_against_type(
        ...
        return await self._execute(lambda: math.prod(
            effic.damage_factor / 100.0
            for ptyp in PokeApi.get_default_pokemon_sync(mon).pokemon_types
    ...
    async def get_mon_matchup_against_move(
        ...
        if move.id == 560:
            result *= await self._execute(lambda: math.prod(
                effic.damage_factor / 100.0
                for ptyp in PokeApi.get_default_pokemon_sync(mon).pokemon_types
                for effic in ptyp.type.type_efficacys__target_type
    ...
    async def get_mon_matchup_against_mon(
        ...
        return [await self .get_mon_matchup_against_type(mon, type_) for type_ in await self.get_mon_types(mon2)]

    async def get_preevo(
            self,
            mon: 'PokeapiModel.classes.PokemonSpecies'
    ) -> typing.Optional['PokeapiModel.classes.PokemonSpecies']:
        return await self._execute(lambda: mon.evolves_from_species)

    async def get_evos(self, mon: 'PokeapiModel.classes.PokemonSpecies'):
        return await self._execute(lambda: mon.evolves_into_species)

    async def get_mon_learnset(self, mon: 'PokeapiModel.classes.PokemonSpecies') -> set['PokeapiModel.classes.Move']:
        return await self._execute(lambda: set(pm.move for pm in PokeApi.get_default_pokemon_sync(mon).pokemon_moves))

    async def get_mon_learnset_with_flags(
            self,
            mon: 'PokeapiModel.classes.PokemonSpecies'
    ) -> list['PokeapiModel.classes.PokemonMove']:
        return await self._execute(lambda: PokeApi.get_default_pokemon_sync(mon).pokemon_moves)

    async def mon_can_learn_move(self, mon: 'PokeapiModel.classes.PokemonSpecies', move: 'PokeapiModel.classes.Move'):
        return await self._execute(
            lambda: PokeApi.get_default_pokemon_sync(mon).pokemon_moves.get(move=move) is not None
        )
    ...
    async def get_mon_abilities_with_flags(
        ...
        return await self._execute(lambda: PokeApi.get_default_pokemon_sync(mon).pokemon_abilities)
    ...
    async def get_mon_abilities(
        ...
        return await self._execute(
            lambda: pab.ability for pab in PokeApi.get_default_pokemon_sync(mon).pokemon_abilities
        )
    ...
    async def mon_has_ability(
        ...
        return await self._execute(
            lambda: PokeApi.get_default_pokemon_sync(mon).pokemon_abilities.get(ability=ability) is not None
        )
    ...
    async def mon_has_type(
        ...
        return await self._execute(
            lambda: PokeApi.get_default_pokemon_sync(mon).pokemon_types.get(type=type_) is not None
        )

    async def has_mega_evolution(self, mon: 'PokeapiModel.classes.PokemonSpecies') -> bool:
        return await self._execute(lambda: mon.pokemons.get(pokemon_forms__is_mega=True) is not None)

    async def get_evo_line(
            self,
            mon: 'PokeapiModel.classes.PokemonSpecies'
    ) -> collection['PokeapiModel.classes.PokemonSpecies']:
        return await self._execute(lambda: mon.evolution_chain.pokemon_species)

    async def has_evos(self, mon: 'PokeapiModel.classes.PokemonSpecies') -> bool:
        return len(await self.get_evo_line(mon)) > 1
    ...
    async def is_in_evo_line(
        ...
        return needle in await self.get_evo_line(haystack)

    async def has_branching_evos(self, mon: 'PokeapiModel.classes.PokemonSpecies') -> bool:
        return await self._execute(
            lambda: any(len(poke.evolves_into_species) > 1 for poke in mon.evolution_chain.pokemon_species)
        )
    ...
    async def mon_is_in_dex(
        ...
        return await self._execute(lambda: mon.pokemon_dex_numbers.get(pokedex=dex) is not None)

    async def get_formes(self, mon: 'PokeapiModel.classes.PokemonSpecies') -> list['PokeapiModel.classes.PokemonForm']:
        return await self._execute(lambda: [poke.pokemon_form for poke in mon.pokemons])

    async def get_default_forme(self, mon: 'PokeapiModel.classes.PokemonSpecies') -> 'PokeapiModel.classes.PokemonForm':
        return await self._execute(lambda: PokeApi.get_default_pokemon_sync(mon).pokemon_form)

    async def get_base_stats(self, mon: 'PokeapiModel.classes.PokemonSpecies') -> dict[str, int]:
        def inner():
            default_mon = PokeApi.get_default_pokemon_sync(mon)
            return {bs.stat.qualified_name: bs.base_stat for bs in default_mon.pokemon_stats}
        return await self._execute(inner)

    async def get_egg_groups(self, mon: 'PokeapiModel.classes.PokemonSpecies') -> list['PokeapiModel.classes.EggGroup']:
        return await self._execute(lambda: [peg.egg_group for peg in mon.pokemon_egg_groups])
    ...
    async def mon_is_in_egg_group(
        ...
        return await self._execute(lambda: mon.pokemon_egg_groups.get(egg_group=egg_group) is not None)

    async def mon_can_mate_with(
            self,
            mon_: 'PokeapiModel.classes.PokemonSpecies',
            mate_: 'PokeapiModel.classes.PokemonSpecies'
    ) -> bool:
        def inner(mon, mate):
            # Babies can't breed
            ...
            mon_egg_groups = [peg.egg_group for peg in mon.pokemon_egg_groups]
            mate_egg_groups = [peg.egg_group for peg in mate.pokemon_egg_groups]
            return any(grp in mon_egg_groups for grp in mate_egg_groups)
        return await self._execute(inner, mon_, mate_)
        ...
        def inner():
            if version:
                return random.choice([
                    txt.flavor_text
                    for txt in mon.pokemon_species_flavor_texts
    ...
    async def get_mon_evolution_methods(
        ...
        return await self._execute(
            lambda: [poke.pokemon_evolution for poke in mon.evolves_into_species]
        )

    async def mon_is_in_undiscovered_egg_group(self, mon: 'PokeapiModel.classes.PokemonSpecies') -> bool:
        return await self._execute(lambda: mon.pokemon_egg_groups.get(egg_group_id=15) is not None)

    async def get_versions_in_group(
            self,
            grp: 'PokeapiModel.classes.VersionGroup'
    ) -> collection['PokeapiModel.classes.Version']:
        return await self._execute(lambda: grp.versions)

    async def get_move_attrs(self, move: 'PokeapiModel.classes.Move') -> list['PokeapiModel.classes.MoveAttribute']:
        return await self._execute(
            lambda: [mam.move_attribute for mam in move.move_attribute_maps]
        )
    ...
    async def get_machines_teaching_move(
        ...
        return await self._execute(lambda: move.machines)

    async def get_version_group_name(self, version_group: 'PokeapiModel.classes.VersionGroup') -> str:
        def inner():
