...
class ShopInventoryPageSource(menus.ListPageSource):
    async def format_page(self, menu: menus.MenuPages, page: list[PokeApi.Item]):
        embed = discord.Embed(
...
class ItemBagPageSource(menus.ListPageSource):
    async def format_page(self, menu: menus.MenuPages, page: list[tuple[PokeApi.Item, int]]):
        embed = discord.Embed(
...
class Shop(BaseCog):
    ...
    @mart.command()
    async def buy(self, ctx: MyContext, item: PokeApi.Item, quantity: int_range(1, 999) = 1):
    ...
    @mart.command()
    async def sell(self, ctx: MyContext, item: PokeApi.Item, quantity: int_range(1, 999) = 1):
    ...
    async def inventory_toss(self, ctx: MyContext, item: PokeApi.Item, quantity: int_range(0, 999)):
